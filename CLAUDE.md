...
- `includes/abilities.php` — **ability layer**, the primary programmatic + CLI entrypoint. Registers `static-site-importer/*` abilities (category `static-site-importer`): `import-url`, `materialize-wordpress-site-plan`, `validate-artifact`, `import-figma`, `export-theme`. CLI commands and REST wraps these same ability functions. When touching import behavior, normalize options here and let entrypoints project them (see recent commits on entrypoint parity).
