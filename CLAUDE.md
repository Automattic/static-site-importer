...
- `static-site-importer.php` — infrastructure-only plugin bootstrap: defines constants, loads the two Blocks Engine transformer entry PHP files, `require_once`s every class in `includes/`, registers REST routes, and loads the ability and WP-CLI surfaces. It registers no admin UI or Gutenberg blocks.
- `includes/abilities.php` — **ability layer**, the primary programmatic + CLI entrypoint. Registers `static-site-importer/*` abilities (category `static-site-importer`): `import`, `materialize-wordpress-site-plan`, `validate-artifact`, `export-theme`. The canonical `import` ability handles HTML, files, ZIP, URL, and Figma sources. CLI commands and REST wrap these same ability functions. When touching import behavior, normalize options here and let entrypoints project them (see recent commits on entrypoint parity).
- `includes/rest.php` — `wp-json/static-site-importer/*` routes (e.g. `import-website-artifact`), thin wrappers around abilities.
- `demos/playground-importer/` — separately packaged README Playground demo plugin containing the `static-site-importer/importer` block. It consumes SSI's public REST contract and is excluded from the SSI runtime package.
