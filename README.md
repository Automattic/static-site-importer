...
- Source HTML is preserved by default. Registered source-exclusion rules remove non-authored platform chrome only when the caller supplies a verified `source_provider_policy` naming that provider, and each removal records selector, provider, reason, and before/after hashes under `source_metadata.collection.source_exclusions`.
