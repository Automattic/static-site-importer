...
Every accepted import materializes final editable block markup into a modern block theme. Use `--operation=plan` to emit a plan response without writes. Apply a saved response with `--operation=apply --plan=/absolute/path/to/plan-response.json`; SSI reads the regular local JSON file and routes it through the canonical ability.
