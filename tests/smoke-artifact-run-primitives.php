...
$json_payload = array( 'rows' => array_fill( 0, 3000, array( 'text' => 'Unicode café / quote "', 'nested' => array( 1.0, true, null, array() ) ) ), 'large' => str_repeat( 'x', 100000 ) );
$json_path = $workspace->publish_json_once( 'buffered.json', $json_payload );
if ( is_wp_error( $json_path ) || file_get_contents( $json_path ) !== wp_json_encode( $json_payload, JSON_PRETTY_PRINT | JSON_PRESERVE_ZERO_FRACTION ) || $json_path !== $workspace->publish_json_once( 'buffered.json', $json_payload ) ) { throw new RuntimeException( 'buffered streaming must preserve exact JSON bytes and immutable replay across buffer boundaries' ); }
...
$metadata_race = new Static_Site_Importer_Artifact_Run_Workspace( $root, 'metadata-race', array( 'expires_at' => gmdate( 'c', time() - 1 ) ) ); unlink( $metadata_race->directory() . '/workspace.json' ); set_error_handler( static function ( int $severity, string $message ): never { throw new RuntimeException( $message ); } ); try { $expired = Static_Site_Importer_Artifact_Run_Workspace::purge_expired_in( $root ); } finally { restore_error_handler(); } if ( ! empty( $expired ) || ! is_dir( $metadata_race->directory() ) ) { throw new RuntimeException( 'expired workspace discovery must tolerate concurrent metadata removal without creating a cleanup receipt' ); } $metadata_race->purge();
