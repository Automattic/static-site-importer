...
$assert( ! is_wp_error( $local_overlay ) && array( 'functions.php', 'assets/js/font-readiness.js' ) === array_column( $local_overlay['writes'], 'target_path' ) && $request_count === count( $GLOBALS['ssi_webfont_requests'] ), 'an authoritative local-font contract emits readiness without falling back to synthesized Google requests' );
...
$assert( ! is_wp_error( $local_overlay_without_diagnostics ) && array( 'functions.php', 'assets/js/font-readiness.js' ) === array_column( $local_overlay_without_diagnostics['writes'], 'target_path' ) && $request_count === count( $GLOBALS['ssi_webfont_requests'] ), 'an authoritative zero-face contract without diagnostics emits readiness while suppressing legacy Google requests' );
