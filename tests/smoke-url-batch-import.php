...
for ( $i = 0; $i < 5 && ! empty( $page_first_resumed['continuation'] ); $i++ ) { $page_first_now = 0; $page_first_resumed = Static_Site_Importer_URL_Batch_Import::import( $page_first_request, array(), $page_first_fetcher, $page_first_importer ); $cursor_files = glob( $page_first_request['work_dir'] . '/.ssi-artifact-run-url-*/batches/*.collection-cursor.json' ) ?: array(); if ( ! empty( $cursor_files ) ) { $cursor_data = json_decode( (string) file_get_contents( $cursor_files[0] ), true ); $cursor_queue_sizes[] = count( $cursor_data['asset_queue'] ?? array() ); } }
