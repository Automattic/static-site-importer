...
if ( is_wp_error( $runtime_one ) || 'completed' !== ( $runtime_one['url_batch_run']['status'] ?? '' ) || empty( $runtime_one['import_id'] ) || is_wp_error( $runtime_replay ) || 'completed' !== ( $runtime_replay['url_batch_run']['status'] ?? '' ) || empty( $ability_replay['success'] ) || 'completed' !== ( $ability_replay['url_batch_run']['status'] ?? '' ) ) { throw new RuntimeException( 'canonical URL apply must resume the existing opaque batch contract' ); }
