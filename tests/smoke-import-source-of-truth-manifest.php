...
$assert( ! is_wp_error( $user_id ), 'user-page-created', is_wp_error( $user_id ) ? $user_id->get_error_message() : '' );
	...
	$result = $import(
		array(
			array(
				'path'    => 'index.html',
				'content' => '<main><h1>Source Of Truth Home</h1></main>',
			),
			array(
				'path'    => 'protected.html',
				'content' => '<main><h1>Protected Replacement</h1></main>',
			),
			array(
				'path'    => 'old.html',
				'content' => '<main><h1>Stale Source Of Truth</h1></main>',
			),
			array(
			...
			array(
				'path'    => 'assets/old-image.txt',
				'content' => 'old generated asset',
			),
		),
	...
	if ( ! is_wp_error( $result ) ) {
		...
		foreach ( $manifest['desired']['files'] ?? array() as $file ) {
			if ( is_array( $file ) ) {
				$desired_kinds[ $file['path'] ?? '' ] = $file['kind'] ?? ''; }
		}
		...
		$previous_manifest['desired']['files'][] = array(
			'path' => 'static-site-importer-batch-bootstrap/preserved.php',
			'kind' => 'theme_bootstrap',
		);
		...
		$reimport = $import(
			array(
				array(
					'path'    => 'index.html',
					'content' => '<main><h1>Source Of Truth Home Updated</h1></main>',
				),
				array(
					'path'    => 'assets/site.css',
		...
		if ( ! is_wp_error( $reimport ) ) {
			...
			$reimport_kinds = array();
			foreach ( $reimport_manifest['desired']['files'] ?? array() as $file ) {
				if ( is_array( $file ) ) {
					$reimport_kinds[ $file['path'] ?? '' ] = $file['kind'] ?? ''; }
			}
