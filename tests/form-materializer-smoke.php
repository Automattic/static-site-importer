...
namespace {
	...
	$select_overlay_rule = null;
	if ( 1 === preg_match( '/\.ssi-node-[a-f0-9]{12} select\{([^}]+)\}/', $all_controls_css, $select_overlay ) ) {
		$select_overlay_rule = $select_overlay[1];
	}
	$assert( is_string( $select_overlay_rule ) && str_contains( $select_overlay_rule, 'padding:8px' ) && str_contains( $select_overlay_rule, 'border:2px solid #222' ) && str_contains( $select_overlay_rule, 'appearance:auto' ) && ! preg_match( '/\.ssi-node-[a-f0-9]{12}\{border:2px solid #222/', $all_controls_css ), 'authored-select-padding-border-and-appearance-reach-the-native-select-not-its-provider-wrapper', (string) $select_overlay_rule );
