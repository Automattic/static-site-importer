...
namespace {
	...
	$unsupported_native_markup = (string) ( $unsupported_native_row['block_markup'] ?? '' );
	$assert( 'mapped' === ( $unsupported_native_row['status'] ?? '' ) && true === ( $unsupported_native_row['runtime_mapped'] ?? false ) && ! str_contains( $unsupported_native_markup, 'wp:group' ) && str_contains( $unsupported_native_markup, 'ssi-runtime-wrapper-0-wrapper-0--email-submit-row' ) && ! in_array( 'provider_native_group_wrapper_unavailable', array_column( $unsupported_native_row['computed_layout_receipt']['losses'] ?? array(), 'reason_code' ), true ), 'jetpack-16-native-fields-carry-proven-runtime-wrapper-mapping-without-core-groups', wp_json_encode( $unsupported_native_row ) );
	$runtime_row_html = Static_Site_Importer_Provider_Form_Runtime_V1::project_runtime_group_wrappers( '<form class="jetpack-contact-form__form"><div class="grunion-field-email-wrap ssi-runtime-wrapper-0-wrapper-0--email-submit-row ssi-runtime-wrapper-0-wrapper-0-hook--ssi-node-0123456789ab"><input type="email"></div><div class="wp-block-button ssi-runtime-wrapper-0-wrapper-0--email-submit-row ssi-runtime-wrapper-0-wrapper-0-hook--ssi-node-0123456789ab"><button type="submit">Subscribe</button></div></form>' );
	$assert( str_contains( $runtime_row_html, '<div class="email-submit-row ssi-node-0123456789ab"><div class="grunion-field-email-wrap"><input type="email"') && str_contains( $runtime_row_html, '<div class="wp-block-button"><button type="submit"') && ! str_contains( $runtime_row_html, 'ssi-runtime-wrapper-' ), 'jetpack-16-rendered-field-and-submit-tree-restores-captured-wrapper-and-keeps-native-controls', $runtime_row_html );
	...
	$scoped_documents_code = 'require ' . var_export( $candidate_transformer, true ) . '; $document = new DOMDocument(); @$document->loadHTML("<div class=\"data-liberation-desktop-document\"><form><div class=\"field-stack\"><input name=\"email\"><button type=\"submit\">Send</button></div></form></div><div class=\"data-liberation-mobile-document\"><form><div class=\"field-stack\"><input name=\"email\"><button type=\"submit\">Send</button></div></form></div>"); $stylesheets = array(array("content" => ".field-stack{display:flex;flex-direction:column;gap:24px}", "source_path" => "desktop.css", "source_hash" => hash("sha256", "desktop")), array("content" => ":where(.data-liberation-mobile-document) .field-stack{display:flex;flex-direction:column;gap:32px}", "source_path" => "mobile.css", "source_hash" => hash("sha256", "mobile"), "media" => "(max-width:768px)")); echo json_encode((new Automattic\\BlocksEngine\\PhpTransformer\\HtmlToBlocks\\Style\\FormLayoutGraphBuilder())->build($document->getElementsByTagName("form")->item(1), $stylesheets));';
	$scoped_documents_json = shell_exec( escapeshellarg( PHP_BINARY ) . ' -r ' . escapeshellarg( $scoped_documents_code ) );
	$scoped_mobile_graph   = is_string( $scoped_documents_json ) ? json_decode( $scoped_documents_json, true ) : array();
	$scoped_mobile_form    = array(
		...
		'control_topology' => array( 'schema' => 'generic/form-control-topology/v1', 'max_depth' => 8, 'max_nodes' => 128, 'truncated' => false, 'nodes' => array( array( 'id' => 'wrapper-0', 'kind' => 'wrapper', 'parent' => null, 'order' => 0, 'depth' => 0, 'tag' => 'div', 'class' => 'field-stack' ), array( 'id' => 'control-0', 'kind' => 'control', 'parent' => 'wrapper-0', 'order' => 0, 'depth' => 1, 'control' => 0 ), array( 'id' => 'control-1', 'kind' => 'control', 'parent' => 'wrapper-0', 'order' => 1, 'depth' => 1, 'control' => 1 ) ) ),
		'layout_graph'     => $scoped_mobile_graph,
	);
	$scoped_mobile_variant   = current( array_filter( $scoped_mobile_graph['variants'] ?? array(), static fn( array $variant ): bool => '(max-width:768px)' === ( $variant['condition']['query'] ?? null ) && '32px' === ( $variant['layout_patch']['gap'] ?? null ) ) ) ?: array();
	$scoped_mobile_validation = Static_Site_Importer_Entity_Materializer_Registry::validate_forms_manifest( array( 'forms' => array( $scoped_mobile_form ) ) );
	$scoped_mobile_row        = Static_Site_Importer_Form_Seeder::seed( array( 'forms' => $scoped_mobile_validation['forms'] ?? array() ) )['forms'][0] ?? array();
	$scoped_mobile_css        = (string) ( $scoped_mobile_row['provider_layout_overlay_css']['css'] ?? '' );
	$assert( is_array( $scoped_mobile_graph ) && false === ( $scoped_mobile_graph['truncated'] ?? null ) && ! empty( $scoped_mobile_variant ) && empty( $scoped_mobile_validation['errors'] ) && str_contains( $scoped_mobile_css, '@media (max-width:768px){' ) && str_contains( $scoped_mobile_css, 'gap:32px' ) && preg_match( '/@media \(max-width:768px\)\{\.ssi-form-[a-f0-9]{12} \.ssi-node-[a-f0-9]{12}\{[^}]*gap:32px/', $scoped_mobile_css ) && ! str_contains( $scoped_mobile_css, 'data-liberation-mobile-document' ), 'paired-BE-to-SSI-mobile-document-form-variant-transposes-proven-gap-to-generated-provider-target', wp_json_encode( array( 'graph' => $scoped_mobile_graph, 'row' => $scoped_mobile_row ) ) );
