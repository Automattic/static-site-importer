...
namespace {
	...
	$assert( empty( $validated_phone_presentation['errors'] ) && 'jetpack_phone_composite' === ( $phone_presentation_target['control_provider'] ?? '' ) && str_contains( (string) ( $phone_presentation_target['control'] ?? '' ), 'ssi-phone-value-' ) && str_contains( (string) ( $phone_presentation_target['control_shell'] ?? '' ), 'ssi-phone-shell-' ) && str_contains( (string) ( $phone_presentation_target['control_carrier'] ?? '' ), 'ssi-phone-carrier-' ) && str_contains( $phone_presentation_css, '--jetpack--contact-form--input-background:#fff' ) && str_contains( $phone_presentation_css, '--jetpack--contact-form--border-color:#1e4b6e' ) && str_contains( $phone_presentation_css, '--jetpack--contact-form--border-radius:0' ) && str_contains( $phone_presentation_css, 'padding-block-start:8px' ) && str_contains( $phone_presentation_css, 'padding-inline-end:8px' ) && str_contains( $phone_presentation_css, 'flex:1 1 0;min-width:0' ) && ! str_contains( $phone_presentation_css, '--jetpack--contact-form--input-padding:' ), 'phone-presentation-splits-shared-shell-chrome-from-logical-tel-padding-and-carrier-sizing', wp_json_encode( array( 'css' => $phone_presentation_css, 'target' => $phone_presentation_target ) ) );
	...
	$phone_composite = Static_Site_Importer_Form_Seeder::project_provider_wrapper_classes( '<div class="grunion-field-phone-wrap ssi-source-wrapper-2--control-shell-wrap"><div class="jetpack-field__input-phone-wrapper ssi-phone-shell-123456789abc ssi-phone-value-123456789abc"><div class="jetpack-combobox-dropdown"><input class="jetpack-combobox-search" type="text"></div><input class="jetpack-field__input-element" type="tel"><input type="hidden" name="full-phone"></div></div>' );
	$assert( str_contains( $phone_composite, 'ssi-phone-shell-123456789abc' ) && str_contains( $phone_composite, '<div class="control-shell ssi-phone-carrier-123456789abc"><input class="jetpack-field__input-element ssi-phone-value-123456789abc" type="tel">' ) && ! str_contains( $phone_composite, 'jetpack-combobox-search ssi-phone-value' ) && ! str_contains( $phone_composite, 'jetpack-combobox-search ssi-phone-carrier' ), 'phone-composite-projection-moves-value-presentation-to-tel-and-sizes-only-its-restored-carrier', $phone_composite );
