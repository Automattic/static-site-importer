...
$payload = array(
	...
	'blocks' => array_map( static fn ( $name ) => array(
		'name' => $name,
		'block_json' => array( 'name' => 'ssi-proof/' . $name, 'title' => $name, 'category' => 'text', 'attributes' => array( 'content' => array( 'type' => 'string', 'default' => '' ) ) ),
...
if ( 'setup' === $phase ) {
	...
	$post_id = wp_insert_post( array( 'post_type' => 'page', 'post_status' => 'publish', 'post_title' => 'Saved owner edit', 'post_content' => $markup ), true );
	$assert( ! is_wp_error( $post_id ), 'owner-edited block persisted as a WordPress page' );
	// A text mention and a similarly named block are not instances of /unused.
	wp_insert_post( array( 'post_type' => 'page', 'post_status' => 'draft', 'post_title' => 'False positive', 'post_content' => '<p>wp:ssi-proof/unused</p><!-- wp:ssi-proof/unused-other /-->' ) );
	update_option( $state_key, array( 'post_id' => $post_id, 'payload' => $payload ) );
	$assert( str_contains( do_blocks( get_post( $post_id )->post_content ), 'Owner edit survives' ), 'actual generated renderer displays the saved edit' );
	...
	$breaking = $payload;
	$breaking['blocks'][0]['block_json']['attributes']['content'] = array( 'type' => 'number', 'default' => 0 );
	...
	$assert( $before_hash === hash_file( 'sha256', $metadata ) && $before === get_post( $state['post_id'] )->post_content, 'installed schema and saved owner content remain byte-identical' );
	...
	$compatible = $payload;
	$compatible['blocks'][1]['block_json']['attributes']['content'] = array( 'type' => 'number', 'default' => 0 );
	...
	update_option( $state_key, array( 'post_id' => $state['post_id'], 'payload' => $compatible ) );
} elseif ( 'reopen' === $phase ) {
	$assert( is_array( $state ), 'saved fixture reopened after replacement in another process' );
	$post = get_post( $state['post_id'] );
	$assert( $markup === $post->post_content, 'original owner edit persists across requests' );
	$assert( str_contains( do_blocks( $post->post_content ), 'Owner edit survives' ), 'actual refreshed renderer still displays the owner edit' );
