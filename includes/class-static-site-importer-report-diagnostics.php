...
class Static_Site_Importer_Report_Diagnostics {
	...
	public static function finalize_quality_report( Static_Site_Importer_Import_Report $report, array $args ): array {
		...
		$quality['unsafe_layout_constraint_count']          = count( array_filter( $report['diagnostics'] ?? array(), static fn( $diagnostic ): bool => is_array( $diagnostic ) && self::UNSAFE_LAYOUT_CONSTRAINT_TYPE === ( $diagnostic['type'] ?? '' ) ) );
		$reasons = array();
		...
		if ( $quality['unsafe_layout_constraint_count'] > 0 ) {
			$reasons[] = self::UNSAFE_LAYOUT_CONSTRAINT_TYPE;
		}
	...
	private static function diagnostic_is_finding_packet_candidate( array $diagnostic ): bool {
		...
		return in_array(
			...
			array(
				...
				self::UNSAFE_LAYOUT_CONSTRAINT_TYPE,
			),
			true
		);
	}
	...
	private static function finding_expected_outcome( array $diagnostic ): string {
		...
		if ( self::UNSAFE_LAYOUT_CONSTRAINT_TYPE === $type ) {
	...
	private static function quality_diagnostic_refs( array $diagnostics ): array {
		$types_by_count = array(
			...
			'unsafe_layout_constraint_count'          => array( self::UNSAFE_LAYOUT_CONSTRAINT_TYPE ),
		);
