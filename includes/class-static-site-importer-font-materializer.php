...
final class Static_Site_Importer_Font_Materializer {
	...
	private static function readiness_script( array $faces ): string {
		...
		return '(async()=>{const faces=' . $faces_json . ';const glyphs="SSI glyph evidence 0123456789";const weight=face=>face.weight.kind==="range"?face.weight.min+" "+face.weight.max:face.weight.value;const probe=face=>new Promise(resolve=>{const descriptor=(face.style||"normal")+" "+weight(face)+" 1em "+JSON.stringify(face.family);let timer;let settled=false;const finish=result=>{if(settled)return;settled=true;clearTimeout(timer);resolve(result)};timer=setTimeout(()=>finish({...face,status:"missing",error:"timeout"}),5000);Promise.resolve().then(()=>document.fonts.load(descriptor,glyphs)).then(()=>finish({...face,status:document.fonts.check(descriptor,glyphs)?"loaded":"missing"}),error=>finish({...face,status:"missing",error:String(error)}))});const results=await Promise.all(faces.map(probe));const readiness={schema:"static-site-importer/font-readiness/v1",status:results.every(face=>face.status==="loaded")?"loaded":"missing",faces:results};window.__staticSiteImporterFontReadiness=readiness;document.documentElement.dataset.staticSiteImporterFontReadiness=readiness.status;let record=document.getElementById("static-site-importer-font-readiness");if(!record){record=document.createElement("script");record.id="static-site-importer-font-readiness";record.type="application/json";document.head.append(record)}record.textContent=JSON.stringify(readiness);})();' . "\n";
	}
