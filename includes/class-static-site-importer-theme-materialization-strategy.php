...
final class Static_Site_Importer_Theme_Materialization_Strategy {
	...
	public static function normalize( array $args ) {
		...
		if ( ! in_array( $strategy, array( self::BLOCK, self::CLASSIC ), true ) ) { return new WP_Error( 'static_site_importer_theme_materialization_invalid', 'theme_materialization must be block or classic.' ); }
		return array( 'strategy' => $strategy, 'evidence' => array( 'schema' => 'static-site-importer/theme-materialization-evidence/v1', 'strategy' => $strategy, 'status' => self::BLOCK === $strategy ? 'selected' : 'pending_compiler_contract' ) );
	}
	public static function classic_contract_evidence( array $compiled ): array {
		$reports = is_array( $compiled['source_reports'] ?? null ) ? $compiled['source_reports'] : array(); $projection = is_array( $reports['classic_theme_projection'] ?? null ) ? $reports['classic_theme_projection'] : array(); $missing = array();
		if ( self::CLASSIC_PROJECTION_SCHEMA !== ( $projection['schema'] ?? null ) ) { $missing[] = 'source_reports.classic_theme_projection schema blocks-engine/classic-theme-projection/v1'; }
		foreach ( array( 'pages', 'shared_chrome', 'stylesheets', 'asset_references', 'runtime_binding_slots' ) as $field ) { if ( ! isset( $projection[ $field ] ) || ! is_array( $projection[ $field ] ) ) { $missing[] = 'source_reports.classic_theme_projection.' . $field; } }
		return array( 'schema' => 'static-site-importer/theme-materialization-evidence/v1', 'strategy' => self::CLASSIC, 'status' => empty( $missing ) ? 'contract_present_unimplemented' : 'blocked_missing_compiler_contract', 'compiler_report_path' => 'source_reports.classic_theme_projection', 'expected_schema' => self::CLASSIC_PROJECTION_SCHEMA, 'missing_data_contract' => $missing, 'fixed_scaffold_schema' => 'static-site-importer/managed-classic-scaffold/v1' );
	}
