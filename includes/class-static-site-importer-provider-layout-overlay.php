...
class Static_Site_Importer_Provider_Layout_Overlay {
	...
	public static function validate_map( mixed $map, array $graph ): array {
		if ( ! is_array( $map ) || self::MAP_SCHEMA !== ( $map['schema'] ?? null ) || ! is_string( $map['provider'] ?? null ) || ! preg_match( '/^[a-z][a-z0-9_-]{0,31}$/D', $map['provider'] ) || ! is_string( $map['scope'] ?? null ) || ! preg_match( '/^\.ssi-form-[a-f0-9]{12}$/D', $map['scope'] ) || ! is_array( $map['targets'] ?? null ) || ! array_is_list( $map['targets'] ) || count( $map['targets'] ) > 128 || array_diff( array_keys( $map ), array( 'schema', 'provider', 'scope', 'targets' ) ) ) return array( 'error' => 'provider layout target map is not a bounded canonical map.' );
		$nodes = array(); foreach ( $graph['nodes'] ?? array() as $node ) if ( is_array( $node ) && is_string( $node['id'] ?? null ) ) $nodes[ $node['id'] ] = true;
		$seen = array(); $targets = array();
		foreach ( $map['targets'] as $target ) {
			if ( ! is_array( $target ) || array_diff( array_keys( $target ), array( 'node', 'selector', 'capabilities' ) ) || ! is_string( $target['node'] ?? null ) || ! isset( $nodes[ $target['node'] ] ) || isset( $seen[ $target['node'] ] ) || ! is_string( $target['selector'] ?? null ) || ! self::safe_selector( $target['selector'], $map['scope'] ) || ! is_array( $target['capabilities'] ?? null ) || ! array_is_list( $target['capabilities'] ) || array_diff( $target['capabilities'], array( 'container_layout', 'direct_child_layout', 'item_layout', 'responsive_layout' ) ) ) return array( 'error' => 'provider layout target map contains an unsafe target.' );
			$seen[ $target['node'] ] = true;
			$targets[] = array( 'node' => $target['node'], 'selector' => $target['selector'], 'capabilities' => array_values( array_unique( $target['capabilities'] ) ) );
		}
	...
	public static function compile( array $graph, mixed $map ): array {
		...
		foreach ( $graph['nodes'] ?? array() as $node ) {
			if ( ! is_array( $node ) || empty( $node['layout'] ) ) continue;
			$id = (string) ( $node['id'] ?? '' ); $target = $targets[ $id ] ?? null;
			if ( null === $target ) { $losses[] = self::loss( 'provider_structure_mismatch', $id ); continue; }
			$declarations = self::declarations( $node['layout'], $target['capabilities'], $id, $losses );
			if ( ! empty( $declarations ) ) { $rules[] = $target['selector'] . '{' . implode( ';', $declarations ) . '}'; $operations[] = array( 'dimension' => 'layout', 'strategy' => 'provider_selector_transposition', 'node_hash' => hash( 'sha256', $id ), 'target_hash' => hash( 'sha256', $target['selector'] ) ); }
		}
		foreach ( $graph['variants'] ?? array() as $variant ) {
			if ( ! is_array( $variant ) || empty( $variant['layout_patch'] ) ) continue;
			$id = (string) ( $variant['node'] ?? '' ); $target = $targets[ $id ] ?? null;
			if ( null === $target || ! in_array( 'responsive_layout', $target['capabilities'], true ) || ! self::safe_condition( $variant['condition'] ?? null ) ) { $losses[] = self::loss( 'responsive_layout_ownership', $id ); continue; }
	...
	private static function safe_value( string $fact, mixed $value ): bool {
		...
		$value = (string) $value;
		if ( '' === $value || strlen( $value ) > 160 || preg_match( '/(?:url\(|[;{}\\\\]|!important|expression\()/i', $value ) ) return false;
		if ( in_array( $fact, array( 'display', 'direction', 'wrap', 'align_items', 'align_content', 'justify_content', 'align_self', 'justify_self' ), true ) ) return (bool) preg_match( '/^(?:flex|grid|block|inline-flex|row|row-reverse|column|column-reverse|wrap|nowrap|wrap-reverse|flex-start|flex-end|center|stretch|baseline|space-between|space-around|space-evenly|start|end)$/D', $value );
		if ( in_array( $fact, array( 'order', 'flex_grow', 'flex_shrink' ), true ) ) return (bool) preg_match( '/^-?[0-9]+(?:\.[0-9]+)?$/D', $value );
	...
	private static function loss( string $reason, string $node ): array { return array( 'dimension' => 'layout', 'reason_code' => $reason, 'node_hash' => hash( 'sha256', $node ) ); }
}
