...
final class Static_Site_Importer_Classic_Theme_Projection {
	...
	public static function build( array $artifact, array $plan ) {
		...
		foreach ( $plan['pages'] ?? array() as $page ) {
			...
			$pages[ $path ] = array(
				'html'        => self::sanitize_html( $fragments['background'] . "\n" . $fragments['main'] ),
				'source_path' => $path,
			);
			if ( $entry === $path ) {
		...
		return array(
			'schema'             => 'static-site-importer/classic-theme-projection/v1',
			'pages'              => $pages,
			'chrome'             => $chrome,
			'chrome_source_path' => $entry,
			'stylesheets'        => $css,
			'bindings'           => array(),
		);
	}
	...
	public static function writes( array $projection, array $resolved, string $theme_uri, string $name ): array {
		...
		foreach ( $scaffold as $path => $content ) {
			$writes[] = array(
				'target_path'             => $path,
				'source_path'             => 'static-site-importer/classic-scaffold/' . $path,
				'kind'                    => 'theme_scaffold',
				'payload'                 => array(
					'encoding' => 'utf8',
					'data'     => $content,
				...
				'reconciliation_identity' => hash( 'sha256', "static-site-importer/classic-scaffold/v1\n" . $path ),
			); }
		return $writes;
	}
	...
	public static function prepare_for_materialization( array $projection, array $resolved ) {
		...
		foreach ( $projection['pages'] as $source => $page ) {
			...
			$pages[ $source ] = array(
				'source_path' => $source,
				'html'        => self::sanitize_html( $page['html'] ),
			);
		}
		$chrome = array(
			'header'     => '',
		...
		return array(
			...
			'pages'              => $pages,
			'chrome'             => $chrome,
			'chrome_source_path' => $chrome_source,
	...
	public static function apply_runtime_bindings( array $projection, array $bindings ) {
		...
		if ( is_wp_error( $result ) ) {
			return $result; }
		foreach ( $result as $identity => $html ) {
			if ( str_starts_with( $identity, 'chrome:' ) ) {
				$projection['chrome'][ substr( $identity, 7 ) ] = $html;
			} else {
				$projection['pages'][ $identity ]['html'] = $html; }
		}
		foreach ( $bindings as $binding ) {
			$id    = (string) ( $binding['reconciliation_identity'] ?? '' );
			$token = '<!--static-site-importer-binding:' . $id . '-->';
			foreach ( $projection['chrome'] ?? array() as $surface => $html ) {
				if ( is_string( $html ) && str_contains( $html, $token ) ) {
					$binding['surface'] = $surface;
				}
			} if ( ! isset( $binding['surface'] ) ) {
				$binding['surface'] = 'page';
			} $projection['bindings'][] = $binding; }
		return $projection;
	}
	...
	private static function transaction( array $projection, array $bindings, bool $apply_replacements ) {
		foreach ( $bindings as $left_index => $left ) {
			foreach ( $bindings as $right_index => $right ) {
				if ( $right_index <= $left_index || ( $left['source_path'] ?? '' ) !== ( $right['source_path'] ?? '' ) ) {
					continue;
				} $a = trim( (string) ( $left['selector'] ?? '' ) );
				$b   = trim( (string) ( $right['selector'] ?? '' ) );
				if ( '' !== $a && '' !== $b && ( $a === $b || str_starts_with( $a, $b . ' ' ) || str_starts_with( $a, $b . '>' ) || str_starts_with( $b, $a . ' ' ) || str_starts_with( $b, $a . '>' ) ) ) {
					return new WP_Error( 'static_site_importer_classic_html_binding_overlap', 'Classic binding selectors may not claim equivalent or ancestor/descendant source nodes.' ); }
			}
		}
		$documents = array();
		foreach ( $bindings as $binding ) {
			$source     = (string) ( $binding['source_path'] ?? '' );
			$surface    = (string) ( $binding['surface'] ?? '' );
			$selector   = $binding['selector'] ?? null;
			$occurrence = $binding['occurrence'] ?? null;
			if ( ! is_string( $selector ) || ! is_int( $occurrence ) || $occurrence < 1 || ( $apply_replacements && ( ! is_string( $binding['replacement_html'] ?? null ) || ! is_array( $binding['render'] ?? null ) ) ) ) {
				return new WP_Error( 'static_site_importer_classic_html_binding_invalid', 'Classic runtime binding is missing its exact source selector and adapter render contract.' ); }
			$query = self::selector_xpath( $selector );
			if ( '' === $query ) {
				return new WP_Error( 'static_site_importer_classic_html_binding_selector_invalid', 'Classic runtime bindings require bounded source-owned selectors.' ); }
			$candidates = '' === $surface ? array( 'page', 'header', 'footer' ) : array( $surface );
			$matches    = array();
			foreach ( $candidates as $candidate ) {
				$identity = 'page' === $candidate ? $source : 'chrome:' . $candidate;
				$html     = 'page' === $candidate ? (string) ( $projection['pages'][ $source ]['html'] ?? '' ) : (string) ( $projection['chrome'][ $candidate ] ?? '' );
				if ( '' === $html ) {
					continue;
				} if ( ! isset( $documents[ $identity ] ) ) {
					$documents[ $identity ] = self::dom( $html );
				} $root = $documents[ $identity ]->getElementById( 'ssi-classic-root' );
				$nodes  = ( new DOMXPath( $documents[ $identity ] ) )->query( $query, $root );
				if ( false !== $nodes && $nodes->length === $occurrence ) {
					$matches[] = array( $identity, $nodes ); }
			}
			if ( 1 !== count( $matches ) ) {
				return new WP_Error(
					'static_site_importer_classic_html_binding_cardinality_mismatch',
					'Classic runtime binding must resolve exactly one page or chrome surface.',
					array(
						'selector' => $selector,
						'surface'  => $surface,
					)
			...
			$nodes       = $matches[0][1];
			$dom         = $documents[ $identity ];
			$root        = $dom->getElementById( 'ssi-classic-root' );
			$replacement = $apply_replacements ? $binding['replacement_html'] : '<div data-static-site-importer-preflight="1" />';
			$fragment    = $dom->createDocumentFragment();
			if ( ! $fragment->appendXML( $replacement ) ) {
				return new WP_Error( 'static_site_importer_classic_html_binding_replacement_invalid', 'Classic runtime binding replacement is not valid fixed provider markup.' ); }
			$target = $nodes->item( $occurrence - 1 );
			if ( ! $target instanceof DOMNode || $target === $root || ! $target->parentNode ) {
				return new WP_Error( 'static_site_importer_classic_html_binding_reserved_target', 'Classic bindings may only replace source-owned descendants.' ); }
			$target->parentNode->replaceChild( $fragment, $target );
		}
		$output = array();
		foreach ( $documents as $source => $dom ) {
			$output[ $source ] = self::root_html( $dom );
		} return $output;
	}
	...
	private static function dom( string $html ): DOMDocument {
		...
		$dom->loadHTML( '<div id="ssi-classic-root">' . $html . '</div>', LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD );
		libxml_clear_errors();
		libxml_use_internal_errors( $previous );
		return $dom; }

	private static function selector_xpath( string $selector ): string {
		if ( preg_match( '/(?:ssi-classic-root|static-site-importer)/i', $selector ) ) {
			return ''; }
		...
		foreach ( $tokens as $token ) {
			...
			$child  = false;
			if ( '' !== ( $m[1] ?? '' ) ) {
				$xpath .= '[local-name()="' . $m[1] . '"]'; }
			if ( '' !== ( $m[2] ?? '' ) ) {
				$xpath .= '[@id="' . $m[2] . '"]'; }
			if ( '' !== ( $m[3] ?? '' ) ) {
				$xpath .= '[contains(concat(" ", normalize-space(@class), " "), " ' . $m[3] . ' ")]'; }
			if ( '' !== ( $m[4] ?? '' ) ) {
				$xpath .= '[count(preceding-sibling::*)=' . ( (int) $m[4] - 1 ) . ']'; }
		}
		return $child ? '' : $xpath;
	}
	...
	private static function sanitize_html( string $html ): string {
		...
		foreach ( $elements as $element ) {
			...
			if ( in_array( $tag, array( 'script', 'iframe', 'object', 'embed', 'base', 'meta' ), true ) || self::unsafe_svg_animation( $tag ) || ( 'style' === $tag && ! self::safe_css( $element->textContent ) ) ) {
			...
			foreach ( $attributes as $attribute ) {
				...
				if ( str_starts_with( $name, 'on' ) || 'srcdoc' === $name || ( 'style' === $name && ! self::safe_css( $attribute->value ) ) ) {
					$element->removeAttributeNode( $attribute );
					continue;
				} if ( in_array( $name, array( 'href', 'src', 'action', 'formaction', 'poster', 'xlink:href' ), true ) && ! self::safe_url( $attribute->value ) ) {
					$element->setAttribute( $attribute->name, '#' );
				} if ( 'srcset' === $name ) {
					$element->setAttribute( 'srcset', self::safe_srcset( $attribute->value ) ); }
			}
		}
	...
	private static function files( array $artifact ): array {
		$files = array();
		foreach ( $artifact['files'] ?? array() as $key => $file ) {
			if ( is_string( $file ) ) {
	...
	private static function canonical_url( string $value ): string {
		$value = html_entity_decode( $value, ENT_QUOTES | ENT_HTML5 ); for ( $i = 0; $i < 3; ++$i ) {
		...
		return strtolower( preg_replace( '/[\x00-\x20\x7f]+/', '', $value ) ?? '' ); }
	...
	private static function unsafe_svg_animation( string $tag ): bool {
	...
	private static function binding_records( array $projection ): array {
		$records = array();
		foreach ( $projection['bindings'] ?? array() as $binding ) {
			$id      = is_array( $binding ) ? (string) ( $binding['reconciliation_identity'] ?? '' ) : '';
			$render  = is_array( $binding ) ? ( $binding['render'] ?? null ) : null;
			$source  = is_array( $binding ) ? (string) ( $binding['source_path'] ?? '' ) : '';
			$surface = is_array( $binding ) ? (string) ( $binding['surface'] ?? 'page' ) : 'page';
			$content = 'page' === $surface ? ( $projection['pages'][ $source ]['html'] ?? '' ) : ( $projection['chrome'][ $surface ] ?? '' );
			if ( preg_match( '/^[a-f0-9]{64}$/', $id ) && is_array( $render ) && in_array( $render['kind'] ?? null, array( 'shortcode', 'blocks' ), true ) && is_string( $render['content'] ?? null ) && is_string( $content ) ) {
				$records[ $id ] = array(
					'kind'        => $render['kind'],
					'content'     => $render['content'],
					'source_path' => $source,
					'surface'     => $surface,
					'page_hash'   => hash( 'sha256', $content ),
				);
			}
