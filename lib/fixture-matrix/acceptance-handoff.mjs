...
import { validateProviderSubmissionEvidence } from './provider-submission-evidence.mjs';
...
export const NOT_PROVEN_REASONS = Object.freeze([
  ...
  'provider_submission_evidence_missing',
  'provider_submission_evidence_invalid',
  'artifact_missing',
  'hash_mismatch',
]);
...
export function produceAcceptanceHandoff(input = {}) {
  ...
  const providerSubmissionRequirementsValue = input.providerSubmissionRequirements || input.provider_submission_requirements;
  const providerSubmissionEvidenceValue = input.providerSubmissionEvidence || input.provider_submission_evidence;
  const providerSubmissionRequirements = artifact('provider_submission_requirements', providerSubmissionRequirementsValue, false);
  const providerSubmissionEvidence = artifact('provider_submission_evidence', providerSubmissionEvidenceValue, false);
  const providerValidation = validateProviderSubmissionEvidence({ fixtureId, requirements: providerSubmissionRequirementsValue, evidence: providerSubmissionEvidenceValue, materializationReceipt: receiptValue, routeEntityMapping: input.routeEntityMapping || input.route_entity_mapping });
  if (providerValidation.required && !providerSubmissionEvidence) reasons.push('provider_submission_evidence_missing');
  else if (!providerValidation.valid) reasons.push('provider_submission_evidence_invalid');
  ...
  const document = {
    ...
    provider_submission_requirements: providerSubmissionRequirements,
    provider_submission_evidence: providerSubmissionEvidence,
    disposition,
...
export function validateAcceptanceHandoff(root, document) {
  ...
  const refs = [document.input_identity, document.compiler_identity, document.wordpress_site_plan, document.terminal_materialization_receipt, document.route_entity_mapping, document.quality_projection, document.provider_submission_requirements, document.provider_submission_evidence, ...array(document.visual_evidence).flatMap((entry) => [entry?.source, entry?.candidate, entry?.diff]), ...array(document.editor_evidence).map((entry) => entry?.evidence)].filter(Boolean);
  ...
  const providerRequirements = readReference(root, document.provider_submission_requirements);
  const providerEvidence = readReference(root, document.provider_submission_evidence);
  ...
  const providerValidation = validateProviderSubmissionEvidence({ fixtureId: document.fixture_id, requirements: providerRequirements, evidence: providerEvidence, materializationReceipt: receipt, routeEntityMapping: mapping });
  if (providerValidation.required && !providerEvidence) reasons.push('provider_submission_evidence_missing');
  else if (!providerValidation.valid) reasons.push('provider_submission_evidence_invalid');
...
export function emitFixtureMatrixAcceptanceHandoffs({ outputDirectory, result }) {
  ...
  for (const fixture of array(result?.fixtures)) {
    ...
    const document = produceAcceptanceHandoff({
      ...
      providerSubmissionRequirements: fixture.provider_submissions,
      providerSubmissionEvidence: fixture.provider_submission_evidence,
    });
