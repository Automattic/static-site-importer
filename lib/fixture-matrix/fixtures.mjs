...
export function normalizeFixture(input) {
  ...
  const providerSubmissions = normalizeArray(manifest?.provider_submissions ?? input.provider_submissions ?? input.providerSubmissions)
    .map((row) => row && typeof row === 'object' && !Array.isArray(row) ? {
      required: row.required === true,
      page_route: String(row.page_route || row.pageRoute || ''),
      form_identity: String(row.form_identity || row.formIdentity || ''),
      provider_id: String(row.provider_id || row.providerId || ''),
      provider_owner: String(row.provider_owner || row.providerOwner || ''),
    } : null)
    .filter(Boolean);
