...
export function discoverFixtures(root, options = {}) {
  ...
  for (const searchRoot of searchRoots) {
    ...
    for (const fixture of inspection.eligible) {
      fixtures.push(normalizeFixture({ root: searchRoot, directory: fixture.directory, entrypoint, fixture_corpus: corpusLabelForSearchRoot(searchRoot, fixtureRoot) }));
    }
  }
...
export function buildFixtureCoverage(root, options = {}, selectedFixtures = [], filter = null) {
  ...
  const inventories = searchRoots.map((searchRoot) => {
    ...
    const selected = inspection.eligible
      .filter((fixture) => selectedIds.has(fixture.id))
      .map(({ id }) => ({ id, reason: 'selected' }));
    const skipped = [
      ...inspection.eligible.filter((fixture) => !selectedIds.has(fixture.id)).map(({ id }) => ({ id, reason: filter ? 'filter_mismatch' : 'omitted' })),
    ...
    return {
      ...
      eligible_fixture_ids: inspection.eligible_ids,
      selected,
      skipped,
      malformed: inspection.malformed.map(({ id, reason }) => ({ id, reason })),
      duplicates: inspection.duplicates.map(({ id, reason }) => ({ id, reason })),
    };
  });
  const active = inventories.find((inventory) => inventory.corpus === 'active') || emptyCoverageInventory('active', fixtureRoot, options);
  const solved = inventories.find((inventory) => inventory.corpus === 'solved') || emptyCoverageInventory('solved', path.join(fixtureRoot, 'solved'), options);
  const invalid = [...inventories.flatMap((inventory) => inventory.malformed), ...inventories.flatMap((inventory) => inventory.duplicates)];
  const omitted = filter ? [] : inventories.flatMap((inventory) => inventory.skipped).filter((fixture) => fixture.reason === 'omitted');
  return {
