...
import { buildFixtureArtifact, buildFixturePolicyArtifact, stageFixtureSource } from './steps/recipe-builder.mjs';
import { buildFixtureSurfaceLineage, buildSurfaceLineageArtifact, surfaceLineageArtifactRef } from './surface-lineage.mjs';
import { emitFixtureMatrixAcceptanceHandoffs } from './acceptance-handoff.mjs';
import { providerSubmissionReportProjection } from './provider-submission-evidence.mjs';
import { emitFixtureMatrixOwnerHandoffs } from './owner-handoff-evidence.mjs';
...
export function normalizeFixtureMatrixResult(input = {}) {
  ...
  const fixtureResults = matrix.fixtures.map((fixture) => {
    const normalized = resultByFixture.get(fixture.id) || normalizeFixtureResult({ fixture_id: fixture.id, fixture_path: fixture.fixture_path, status: 'not_run' });
    return attachFixtureTaxonomy({ ...normalized, surface_lineage: buildFixtureSurfaceLineage(fixture, normalized, input) }, fixture);
  });
  ...
  const acceptableLossClassCounts = countBy(findings.filter((finding) => finding.loss_acceptance === 'acceptable'), (finding) => finding.loss_class || 'unsupported_loss');
  const unacceptableLossClassCounts = countBy(findings.filter((finding) => finding.loss_acceptance !== 'acceptable'), (finding) => finding.loss_class || 'unsupported_loss');
  ...
  return {
    ...
    summary: {
      ...
      acceptable_loss_classes: acceptableLossClassCounts,
      unacceptable_loss_classes: unacceptableLossClassCounts,
...
function attachFixtureTaxonomy(result, fixture) {
  ...
  const providerSubmissions = normalizeArray(fixture.provider_submissions || result.provider_submissions);
  const providerSubmission = providerSubmissionReportProjection({
    fixtureId: result.fixture_id,
    requirements: providerSubmissions,
    evidence: result.provider_submission_evidence,
    materializationReceipt: result.matrix_evidence?.materialization_receipt,
    routeEntityMapping: providerRouteEntityMapping(result.surface_lineage),
  });
  const importReport = objectValue(result.import_report);
  return {
    ...result,
    ...
    ...(providerSubmissions.length > 0 ? { provider_submissions: providerSubmissions } : {}),
    ...(providerSubmission ? { import_report: { ...importReport, provider_submission_verification: providerSubmission } } : {}),
    taxonomy: {
      ...taxonomy,
      ...result.taxonomy,
      fixture_class: fixtureClass,
...
function providerRouteEntityMapping(surfaceLineage) {
  return normalizeArray(surfaceLineage).map((surface) => ({ route: surface?.surface?.source_entry, entity: String(surface?.materialized_document?.post_id ?? '') })).filter((row) => row.route && row.entity);
}
...
export function normalizeFixtureResult(input) {
  ...
  return {
    ...
    editor_presentation: boundBlob(input.editor_presentation || input.editorPresentation || null),
    editor_interaction: boundBlob(input.editor_interaction || input.editorInteraction || null),
    ...
    provider_submission_evidence: boundBlob(normalizeArray(input.provider_submission_evidence || input.providerSubmissionEvidence)),
    svg_font_embedding_evidence: boundBlob(input.svg_font_embedding_evidence || input.svgFontEmbeddingEvidence || null),
    policy_rejections: boundBlob(input.policy_rejections || input.policyRejections || null),
...
export function writeFixtureMatrixArtifacts(input = {}) {
  ...
  for (const fixture of matrix.fixtures) {
    ...
    writeJsonFile(path.join(fixtureDirectory, 'policy-rejection-artifact.json'), buildFixturePolicyArtifact(fixture, input));
  ...
  return {
    ...
    metadata: {
      ...
      source_staging: {
        ...
        reason: stageSources ? 'source_runtime_evidence_enabled' : 'visual_live_wp_and_runtime_presentation_evidence_disabled',
      },
    },
    artifact_refs: [
      ...
      artifactRef('fixture-coverage', path.join(outputDirectory, 'fixture-coverage.json'), 'summary'),
...
export function writeFixtureMatrixResultArtifacts(input = {}) {
  ...
  // Finding packets are a public legacy array artifact. Coverage is emitted as a
  // separate additive artifact so existing consumers need no migration.
  writeJsonFile(path.join(outputDirectory, 'finding-packets.json'), result.findings);
  writeJsonFile(path.join(outputDirectory, 'fixture-coverage.json'), result.summary.fixture_coverage || null);
  ...
  writeJsonFile(path.join(outputDirectory, 'acceptance-handoffs.json'), emitFixtureMatrixAcceptanceHandoffs({ outputDirectory, result }));
  writeJsonFile(path.join(outputDirectory, 'owner-handoffs.json'), emitFixtureMatrixOwnerHandoffs({ outputDirectory, result }));
  return result;
}
...
function shouldStageFixtureSources(input = {}) {
  return input.visualParity !== false
    ...
    || input.runtimePresentationEvidence === true
    || input.runtime_presentation_evidence === true;
}
