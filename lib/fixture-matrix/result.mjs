...
export function normalizeFixtureMatrixResult(input = {}) {
  ...
  return {
    ...
    summary: {
      ...
      pending_runtime: gatedFixtureResults.filter((result) => result.status === 'pending_runtime').length,
...
function applyFixtureQualityGate(result, findings, options = {}) {
  ...
  if (result.status === 'pending_runtime') {
    return {
      ...result,
      raw_status: result.status,
      success: false,
...
export function normalizeFixtureResult(input) {
  ...
  return {
    ...
    runtime: boundBlob(input.runtime || null),
...
function qualityBudgetSummaries(classRollups) {
  return Object.fromEntries(Object.entries(classRollups).map(([key, row]) => {
    ...
    return [key, {
      ...
      passed: row.passed,
      failed: row.failed,
      pending_runtime: row.pending_runtime,
      not_run: row.not_run,
