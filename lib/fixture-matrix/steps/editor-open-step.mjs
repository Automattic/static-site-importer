...
export function editorOpenStep(input = {}) {
  ...
  const requestedPresentationUrl = firstPresent([input.presentationUrl, input.presentation_url, surface?.target, fixture.target]);
  const presentationUrl = requestedPresentationUrl === 'front-page' ? '/' : requestedPresentationUrl || '/';
  ...
  args.push(`presentation-url=${presentationUrl}`);
  args.push('presentation-frontend-selector=.wp-block-post-content');
