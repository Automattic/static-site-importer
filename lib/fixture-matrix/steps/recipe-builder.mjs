...
function woocommerceOnboardingSuppressionStep(fixture) {
  return {
    ...
    allowFailure: true,
    args: [
      `command=eval ${shellToken("global $wpdb; delete_transient('_wc_activation_redirect'); update_option('woocommerce_onboarding_profile', array('completed' => true, 'skipped' => true)); update_option('woocommerce_task_list_hidden', 'yes'); $key = $wpdb->get_blog_prefix() . 'persisted_preferences'; $preferences = get_user_meta(1, $key, true); $preferences = is_array($preferences) ? $preferences : array(); $editor = is_array($preferences['core/edit-post'] ?? null) ? $preferences['core/edit-post'] : array(); $preferences['core/edit-post'] = array_merge($editor, array('welcomeGuide' => false, 'welcomeGuideTemplate' => false)); update_user_meta(1, $key, $preferences);")}`,
    ],
