...
import { editorBlockValidationStep, editorChromeValidationStep } from './editor-validation-step.mjs';
...
// Stage a fixture's static source (index.html + css/js/images) into the matrix
// artifacts tree so the in-sandbox WordPress origin can serve it for the
...
// JS, and images exactly as the original did. Both the staged source and the
// fixture's `artifact.json` import payload carry the same unmodified source HTML,
// so neither side can drift from the other. Returns the staged paths.
...
export function stageFixtureSource(fixture, fixtureDirectory, options = {}) {
  ...
  for (const file of files) {
    ...
    if (isHtmlPath(file.relative_path)) {
      fs.writeFileSync(destination, rewriteStagedSourceHtml(fs.readFileSync(file.absolute_path, 'utf8'), file.relative_path));
...
function fixtureWorkflowSteps(options) {
  ...
  return [
    ...
    ...surfaces.flatMap((surface, index) => [
      ...
      ...(editorValidationEnabled ? [editorChromeValidationStep({ fixture, surface: editorSurface(surface), ...editorValidationOptions })] : []),
