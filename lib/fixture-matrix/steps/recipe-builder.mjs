...
function providerReadinessStep(fixture) {
  const code = "$blocks = WP_Block_Type_Registry::get_instance(); $required = array('jetpack/contact-form','jetpack/field-text','jetpack/field-email'); $missing = array(); foreach ($required as $name) { if (!$blocks->is_registered($name)) { $missing[] = $name; } } $ready = empty($missing); WP_CLI::line(wp_json_encode(array('schema'=>'static-site-importer/provider-readiness/v1','ready'=>$ready,'missing_block_types'=>$missing,'jetpack_class'=>class_exists('Automattic\\Jetpack\\Forms\\ContactForm\\Contact_Form')))); if (!$ready) { WP_CLI::error('Required provider blocks are not registered.'); }";
