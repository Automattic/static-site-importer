...
export function buildFixtureMatrixRecipe(input = {}) {
  ...
  const hostStagedDependencies = Boolean(input.dependencyPlan || input.dependency_plan);
...
function fixtureWorkflowSteps(options) {
  ...
  return [
    ...
    ...(hostStagedDependencies ? [] : [prepareFixtureDependenciesStep(fixture, commandArtifactsDirectory, attemptId, runtimePresentationEvidence ? RUNTIME_PRESENTATION_EVIDENCE_ARTIFACT_FILENAME : 'artifact.json')]),
    importFixtureStep(fixture, commandArtifactsDirectory, runId, attemptId, runtimePresentationEvidence ? RUNTIME_PRESENTATION_EVIDENCE_ARTIFACT_FILENAME : 'artifact.json', hostStagedDependencies),
...
function importFixtureStep(fixture, commandArtifactsDirectory, runId, attemptId, artifactFilename = 'artifact.json', hostStagedDependencies = false) {
...
export function hostStageDependencyPlan(plan) {
  ...
  return plan.entries.map((entry) => {
    ...
    if (entry.host_resolution && (!/^[a-f0-9]{64}$/i.test(entry.host_resolution.archive_sha256 || '') || entry.host_resolution.activation_status !== 'activated' || !entry.host_resolution.runtime_id)) {
      throw new Error('SSI dependency plan contains an incomplete host resolution receipt.');
    }
    return {
      source: `https://downloads.wordpress.org/plugin/${entry.package}.latest-stable.zip`,
      slug: entry.slug,
      pluginFile: entry.plugin_entrypoint,
      ...(entry.host_resolution ? { sha256: entry.host_resolution.archive_sha256 } : {}),
      activate: true,
      metadata: {
