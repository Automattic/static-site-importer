...
export function runtimePresentationEvidenceProbeStep({ fixture = {}, sourceUrl, artifact, outputArtifact, outputRuntimePath, viewport = { width: 1280, height: 1600 } } = {}) {
  const entrypoint = String(fixture.entrypoint || 'index.html').replace(/^\/+/, '');
...
export function runtimePresentationEvidenceMergeStep({ fixture = {}, artifactRoot, outputArtifact, mergedArtifactFilename = RUNTIME_PRESENTATION_EVIDENCE_ARTIFACT_FILENAME } = {}) {
  const relativePath = outputArtifact || runtimePresentationEvidenceArtifactPath(fixture);
  const config = Buffer.from(JSON.stringify({ artifact_root: artifactRoot, fixture_id: fixture.id, output_artifact: relativePath, merged_artifact_filename: mergedArtifactFilename }), 'utf8').toString('base64');
...
if ( ! is_array( $config ) || ! preg_match( '/^[A-Za-z0-9][A-Za-z0-9._-]{0,79}$/', (string) ( $config['fixture_id'] ?? '' ) ) || ! preg_match( '#^[A-Za-z0-9][A-Za-z0-9._-]{0,79}/runtime-presentation-evidence\\.json$#', (string) ( $config['output_artifact'] ?? '' ) ) || '${RUNTIME_PRESENTATION_EVIDENCE_ARTIFACT_FILENAME}' !== ( $config['merged_artifact_filename'] ?? null ) ) { $fail( 'invalid_runtime_presentation_evidence_path', 'Runtime presentation evidence paths are invalid.' ); }
