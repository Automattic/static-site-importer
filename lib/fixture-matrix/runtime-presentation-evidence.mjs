...
export function runtimePresentationEvidenceProbeStep({ fixture = {}, sourceUrl, artifact, outputArtifact, viewport = { width: 1280, height: 1600 } } = {}) {
  ...
  return { command: 'wordpress.browser-probe', allowFailure: true, args: [`url=${sourceUrl}`, 'wait-for=networkidle', `viewport=${viewport.width}x${viewport.height}`, `script=${script}`, `output-artifact=${outputPath}`, 'capture=html'], metadata: { fixture_id: fixture.id, fixture_path: fixture.fixture_path || fixture.directory, phase: 'runtime-presentation-evidence', evidence_schema: RUNTIME_PRESENTATION_EVIDENCE_SCHEMA, output_artifact: outputPath } };
}
