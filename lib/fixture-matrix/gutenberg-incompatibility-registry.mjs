...
function editorPresentationStatusForFixture(fixture) {
  ...
  if (editorPresentationEvidenceComplete(presentation, fixture)) return 'passed';
  if (Object.keys(presentation).length > 0) return 'missing_styles';
  return 'not_evaluated';
}
...
export function editorPresentationEvidenceComplete(value, fixture = {}) {
  ...
  const completeExpectedIdentities = presentation.schema === EDITOR_PRESENTATION_SCHEMA
...
function acceptanceDecision({ frontendVisualStatus, editorCanvasStatus, editorPresentationStatus, editorValidityStatus, nativeEditabilityStatus, visibleRuntimeOrHtmlIslands, gutenbergGapPatterns, transformerGapPatterns, visualOnlyPatterns, runtimeIslandPatterns, editorRiskPatterns }) {
  ...
  if (editorValidityStatus === 'invalid_blocks' || editorCanvasStatus === 'diverged' || editorPresentationStatus === 'missing_styles' || editorRiskPatterns.length > 0) {
  ...
  return { solved_candidate: true, status: 'solved_candidate', reason: 'passed frontend visual parity, editor presentation coverage, block validity, and native editability without limitation patterns' };
}
