...
const EDITOR_PRESENTATION_SCHEMA = 'static-site-importer/editor-presentation-evidence/v2';
const CURRENT_EDITOR_PRESENTATION_SCHEMA = 'static-site-importer/editor-presentation-evidence/v3';
const LEGACY_EDITOR_PRESENTATION_SCHEMA = 'static-site-importer/editor-presentation-evidence/v1';
const EDITOR_PRESENTATION_PROVIDER_SCHEMA = 'wp-codebox/editor-presentation/v1';
const EDITOR_VALIDATION_SCHEMA = 'wp-codebox/editor-validate-blocks/v1';
...
const PATTERNS = [
  ...
  pattern('semantic-description-list', 'Semantic description-list markup using paired dt and dd terms that must retain its authored relationship and layout.', 'core_html', 'Core blocks have no description-list block or paired term-definition editing model; list and table blocks do not preserve dt/dd semantics.', 'custom-block-candidate', true, /<d[ldt]\b|\bdescription[-_\s]?list\b|\bdefinition[-_\s]?(?:list|term)\b/i),
...
export function renderGutenbergIncompatibilityRegistryMarkdown(registry = {}) {
  ...
  lines.push('', '## Fixture Decisions', '', '| Fixture | Acceptance | Frontend Visual | Editor Canvas | Editor Presentation | Block Validity | Native Editability | Runtime/HTML Islands | Gutenberg Gaps | Visual-only Patterns | Reason |', '| --- | --- | --- | --- | --- | --- | --- | ---: | --- | --- | --- |');
  for (const row of normalizeArray(registry.fixture_decisions)) {
    lines.push(`| \`${row.fixture_id}\` | ${row.acceptance_status || '(unknown)'} | ${row.frontend_visual_status || '(unknown)'} | ${row.editor_canvas_status || '(unknown)'} | ${row.editor_presentation_status || '(unknown)'} | ${row.block_validity_status || row.editor_validity_status || '(unknown)'} | ${row.native_editability_status} | ${row.visible_runtime_or_html_islands || 0} | ${(row.gutenberg_gap_patterns || []).map((key) => `\`${key}\``).join(', ') || '(none)'} | ${(row.visual_only_patterns || []).map((key) => `\`${key}\``).join(', ') || '(none)'} | ${table(row.solved_candidate_reason || row.acceptance_reason || '') || '(none)'} |`);
  }
...
function fixtureDecision(fixture, fixtureFindings, fixturePatterns) {
  ...
  const editorPresentationStatus = editorPresentationStatusForFixture(fixture);
  ...
  return compactObject({
    ...
    editor_presentation_status: editorPresentationStatus,
...
function editorPresentationStatusForFixture(fixture) {
  const presentation = objectValue(fixture.editor_presentation || fixture.editorPresentation);
  const interaction = objectValue(fixture.editor_interaction || fixture.editorInteraction);
  if (editorPresentationEvidenceComplete(presentation, fixture) && editorInteractionEvidenceComplete(interaction)) return 'passed';
  if (Object.keys(presentation).length === 0) return 'not_evaluated';
  if (!stylesheetCoverageComplete(presentation, fixture) || !matchedEditorRenderingEvidencePresent(presentation) || Object.keys(interaction).length === 0) return 'not_evaluated';
  if (!matchedEditorRenderingComplete(presentation)) return 'rendering_mismatch';
  if (!editorInteractionEvidenceComplete(interaction)) return 'interaction_failed';
  return 'not_evaluated';
}

// Promotion evidence is a derived set relation, not a provider-supplied boolean.
// Keep the check here so solved-candidate classification and receipt verification
// use the same fail-closed contract.
export function editorPresentationEvidenceComplete(value, fixture = {}) {
  const presentation = objectValue(value);
  return stylesheetCoverageComplete(presentation, fixture)
    && matchedEditorRenderingComplete(presentation);
}

function stylesheetCoverageComplete(presentation, fixture) {
  const expected = normalizeArray(presentation.expected_identities);
  const observed = normalizeArray(presentation.observed_identities);
  const missing = normalizeArray(presentation.missing_identities);
  const canvasDocumentType = presentation.canvas_document_type;
  const canvasDocumentCountValid = canvasDocumentType === 'iframe'
    ? positiveIntegerValue(presentation.iframe_count)
    : canvasDocumentType === 'parent' && presentation.iframe_count === 0;
  const completeExpectedIdentities = [CURRENT_EDITOR_PRESENTATION_SCHEMA, EDITOR_PRESENTATION_SCHEMA].includes(presentation.schema)
    ? presentation.expected_identities_complete === true
    : presentation.schema === LEGACY_EDITOR_PRESENTATION_SCHEMA
      && legacyExpectedIdentitiesComplete(presentation, fixture);
  if (!completeExpectedIdentities
    || presentation.provider_schema !== EDITOR_PRESENTATION_PROVIDER_SCHEMA
    || !canvasDocumentCountValid
    || expected.length === 0
    || !identitySet(expected)
    || !identitySet(observed)
    || !identitySet(missing)
    || Number(presentation.expected_identity_count) !== expected.length
    || Number(presentation.observed_identity_count) !== observed.length) {
    return false;
  }
  const expectedSet = new Set(expected);
  const observedSet = new Set(observed);
  const expectedMissing = expected.filter((identity) => !observedSet.has(identity));
  return presentation.coverage_complete === true
    && expectedMissing.length === 0
    && missing.length === 0
    && [...expectedSet].every((identity) => observedSet.has(identity));
}

function matchedEditorRenderingComplete(presentation) {
  if (!matchedEditorRenderingEvidencePresent(presentation)) return false;
  const idle = objectValue(presentation.idle_canvas || presentation.idleCanvas);
  const matched = objectValue(presentation.matched_rendering || presentation.matchedRendering);
  return idle.status === 'captured'
    && Number(idle.onboarding_modal_count) === 0
    && matched.status === 'passed'
    && matched.equivalent_canvas_widths === true
    && matched.major_geometry_drift === false
    && matched.unreadable_content === false
    && matched.hidden_content === false
    && Number(matched.unresolved_asset_count) === 0;
}

function matchedEditorRenderingEvidencePresent(presentation) {
  if (presentation.schema !== CURRENT_EDITOR_PRESENTATION_SCHEMA) return false;
  const idle = objectValue(presentation.idle_canvas || presentation.idleCanvas);
  const matched = objectValue(presentation.matched_rendering || presentation.matchedRendering);
  return idle.schema === 'wp-codebox/editor-idle-canvas/v1'
    && typeof idle.status === 'string' && idle.status.length > 0
    && Number.isInteger(Number(idle.onboarding_modal_count)) && Number(idle.onboarding_modal_count) >= 0
    && matched.schema === 'wp-codebox/editor-presentation-match/v1'
    && typeof matched.status === 'string' && matched.status.length > 0
    && typeof matched.equivalent_canvas_widths === 'boolean'
    && typeof matched.major_geometry_drift === 'boolean'
    && typeof matched.unreadable_content === 'boolean'
    && typeof matched.hidden_content === 'boolean'
    && Number.isInteger(Number(matched.unresolved_asset_count)) && Number(matched.unresolved_asset_count) >= 0
    && ['frontend_screenshot', 'editor_screenshot', 'diff_screenshot'].every((key) => typeof matched[key] === 'string' && matched[key].length > 0);
}

export function editorInteractionEvidenceComplete(value) {
  const interaction = objectValue(value);
  return interaction.schema === 'static-site-importer/editor-interaction-evidence/v1'
    && interaction.provider_schema === 'wp-codebox/editor-actions/v1'
    && interaction.selection?.status === 'ok'
    && interaction.text_mutation?.status === 'ok'
    && interaction.text_mutation?.mutation_status === 'applied'
    && interaction.block_movement?.status === 'ok'
    && interaction.block_movement?.mutation_status === 'applied'
    && interaction.save?.schema === 'wp-codebox/editor-save/v1'
    && interaction.save?.status === 'saved'
    && interaction.save?.marker_present === true
    && interaction.reload?.status === 'ok'
    && interaction.post_save_validation?.schema === 'wp-codebox/editor-validity/v1'
    && interaction.post_save_validation?.status === 'clean';
}

// v1 did not persist a completeness marker. It remains acceptable only when the
// retained raw site plan has a matching declared count and reproduces its exact
// expected CSS identity set; bounded summaries are not sufficient provenance.
function legacyExpectedIdentitiesComplete(presentation, fixture) {
  const report = objectValue(fixture.import_report || fixture.importReport || fixture.report);
  const blocksEngine = objectValue(report.blocks_engine || report.blocksEngine);
  ...
  const declaredCount = Number(plan.asset_count ?? plan.assetCount);
  if (!Number.isInteger(declaredCount)
    || declaredCount !== assets.length
    || plan.assets_truncated === true
    || plan.assetsTruncated === true) {
    return false;
  }
  const expectedFromPlan = [...new Set(assets.filter((asset) => {
    if (asset?.kind !== 'css') return false;
    const scopes = normalizeArray(asset.scopes);
    return scopes.length === 0 || scopes.some((scope) => scope?.kind === 'global' || scope?.front_page === true);
  }).map((asset) => String(asset.content_hash || asset.contentHash || asset.payload_sha256 || '').toLowerCase()).filter((identity) => /^[a-f0-9]{64}$/.test(identity)))].sort();
  const expected = normalizeArray(presentation.expected_identities);
  return expectedFromPlan.length > 0
    && expectedFromPlan.length === expected.length
    && expectedFromPlan.every((identity, index) => identity === expected[index]);
}

function acceptanceDecision({ frontendVisualStatus, editorCanvasStatus, editorPresentationStatus, editorValidityStatus, nativeEditabilityStatus, visibleRuntimeOrHtmlIslands, gutenbergGapPatterns, transformerGapPatterns, visualOnlyPatterns, runtimeIslandPatterns, editorRiskPatterns }) {
  ...
  if (frontendVisualStatus === 'not_evaluated' || editorCanvasStatus === 'not_captured' || editorPresentationStatus === 'not_evaluated' || editorValidityStatus === 'not_validated') {
    return { solved_candidate: false, status: 'evidence_gap', reason: 'required frontend visual, editor canvas, editor presentation, or block-validity evidence is missing' };
  }
  if (editorValidityStatus === 'invalid_blocks' || editorCanvasStatus === 'diverged' || ['missing_styles', 'rendering_mismatch', 'interaction_failed'].includes(editorPresentationStatus) || editorRiskPatterns.length > 0) {
  ...
  return { solved_candidate: true, status: 'solved_candidate', reason: 'passed frontend visual parity, matched editor presentation and interaction, block validity, and native editability without limitation patterns' };
}
...
function editorInvalidCountForFixture(fixture, fixtureFindings) {
  return numberValue(fixture.editor_quality?.editor_invalid_count || fixture.editor_quality?.invalid_block_count || fixture.editor_validation?.invalid_blocks || fixture.editor_validation?.invalid_block_count || fixture.editor_validation?.invalid_count)
...
function editorValidatedForFixture(fixture) {
  const validation = objectValue(fixture.editor_validation || fixture.editorValidation);
  const total = Number(validation.total_blocks);
  return validation.schema === EDITOR_VALIDATION_SCHEMA
    && validation.validation_method === 'wp.blocks.validateBlock'
    && validation.validation_provider === 'wordpress-block-editor'
    && validation.content_source === 'edited-post-content'
    && positiveIntegerValue(validation.block_types_registered)
    && positiveIntegerValue(total)
    && Number.isInteger(Number(validation.valid_blocks))
    && Number.isInteger(Number(validation.invalid_blocks))
    && Number.isInteger(Number(validation.result_count))
    && validation.results_complete === true
    && Number(validation.result_count) === total
    && Number(validation.valid_blocks) === total
    && Number(validation.invalid_blocks) === 0;
}
...
function identitySet(values) {
  return values.every((value) => typeof value === 'string' && /^[a-f0-9]{64}$/.test(value))
