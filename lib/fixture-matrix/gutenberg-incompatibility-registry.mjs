...
function pattern(patternKey, description, fallbackKind, impossibleInCoreReason, baseClassification, noCoreBlockPath, match) {
  return { pattern_key: patternKey, description, fallback_kind: fallbackKind, impossible_in_core_reason: impossibleInCoreReason, base_classification: baseClassification, no_core_block_path: noCoreBlockPath, match };
}
...
function fixtureDecision(fixture, fixtureFindings, fixturePatterns) {
  ...
  const editorValidityStatus = editorValidityStatusForFixture(editorInvalidCount, editorValidated);
...
function classificationRank(value) {
  if (value === 'custom-block-candidate') {
    return 0;
  }
...
function editorValidityStatusForFixture(editorInvalidCount, editorValidated) {
  if (editorInvalidCount > 0) {
    return 'invalid_blocks';
  }
  return editorValidated ? 'valid' : 'not_validated';
}
