...
export function collectVisualParityDiagnostics(payload, options = {}) {
  ...
  for (const comparison of collectVisualParityComparisons(payload, options)) {
    if (typeof comparison.source_title === 'string'
      && typeof comparison.candidate_title === 'string'
      && comparison.source_title !== comparison.candidate_title) {
      diagnostics.push({
        kind: DOCUMENT_TITLE_MISMATCH_KIND,
        loss_class: DOCUMENT_TITLE_MISMATCH_KIND,
        source_path: comparison.source_path || '',
        expected_output: comparison.source_title,
        observed_output: comparison.candidate_title,
        message: `Rendered document title mismatch: expected "${comparison.source_title}", observed "${comparison.candidate_title}".`,
      });
    }
...
function mergeVisualParityComparison(left, right) {
  return {
    ...
    source_title: left.source_title ?? right.source_title,
    candidate_title: left.candidate_title ?? right.candidate_title,
  };
}
...
function normalizeVisualParityComparison(value, options = {}) {
  ...
  const candidateObject = objectValue(obj.candidate);
  ...
  const normalized = {
    ...
    source_title: typeof sourceObject.title === 'string' ? sourceObject.title : undefined,
    candidate_title: typeof candidateObject.title === 'string' ? candidateObject.title : undefined,
