...
function normalizeCollectedFixtureResult({ fixture, payloads, fixtureArtifactsDirectory, codeboxError, visualParity, liveWpParity, dependencyOverrides, sidecar }) {
  ...
  return normalizeFixtureResult({
    ...
    editor_presentation: collectEditorPresentation(merged),
...
export function collectEditorPresentation(payload) {
  ...
  const expectedIdentities = [...new Set(normalizeArray(plan.assets).filter((asset) => asset?.kind === 'css' && normalizeArray(asset.scopes).some((scope) => scope?.kind === 'global' || scope?.front_page === true)).map((asset) => String(asset.content_hash || asset.contentHash || '').toLowerCase()).filter((value) => /^[a-f0-9]{64}$/.test(value)))].sort();
