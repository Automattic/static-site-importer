...
export function collectFixtureMatrixRunResults(input = {}) {
  ...
  const results = matrix.fixtures.map((fixture) => {
    ...
    const payloads = [
      ...
      ...(sidecar.status === 'verified' ? [materializationSidecarPayload(fixture.id, sidecar.payload)] : []),
    ];
...
function normalizeCollectedFixtureResult({ fixture, payloads, fixtureArtifactsDirectory, codeboxError, visualParity, liveWpParity, dependencyOverrides, sidecar }) {
  ...
  const runtimePresentationEvidence = collectRuntimePresentationEvidence(payloads);
  const visualParityOptions = { ...visualParity, fixtureArtifactsDirectory };
  const diagnostics = attachDiagnosticLineage([...collectFixtureDiagnostics(merged, { visualParity: visualParityOptions }), ...pendingRuntimeDiagnostics(merged), ...runtimePresentationEvidence.diagnostics], payloads);
  ...
  return normalizeFixtureResult({
    ...
    status: fixtureStatus(payloads.length, error, success, merged),
    success,
    error,
...
function materializationSidecarPayload(fixtureId, sidecar) {
  const receipt = objectValue(sidecar.receipt);
  const status = sidecarStatus(receipt.status);
  return {
    ...
    runtime: status === 'pending_runtime' ? { status: 'resume_required' } : undefined,
    materialization_runtime_sidecar: sidecar,
...
function pendingRuntimeDiagnostics(payload) {
  if (payload.status !== 'pending_runtime') {
    return [];
  }
  const receipt = objectValue(payload.materialization_receipt || payload.materializationReceipt);
  const reasonCode = firstString([payload.reason_code, payload.reasonCode, receipt.reason_code, receipt.reasonCode, 'runtime_resume_required']);
  const message = firstString([payload.message, receipt.message, `Validation is resumable after a fresh WordPress runtime (${reasonCode}).`]);
  return [{
...
function inferredDiagnosticBoundary(diagnostic) {
  ...
  if (['importer_materialization_bug', 'missing_asset', 'dropped_images', 'runtime_resume_required'].includes(kind) || ['importer_materialization_bug', 'missing_asset', 'runtime_resume_required'].includes(lossClass)) return 'materialization';
  return '';
}
...
export function collectMatrixEvidence(payload, options = {}) {
  ...
  const receiptEvidence = compactObject({
    schema: materializationReceipt.schema,
    status: materializationReceipt.status,
    ...(materializationReceipt.status === 'pending_runtime' ? {
      reason_code: materializationReceipt.reason_code || materializationReceipt.reasonCode,
    } : {
  ...
  return {
    ...
    materialization_receipt: receiptEvidence,
...
function validSidecar(row) {
  ...
  return statusesAgree(row.provenance.provider_status, receipt.status, row.command_result?.status);
}

function statusesAgree(provenanceStatus, receiptStatus, commandStatus) {
  return provenanceStatus === receiptStatus && (commandStatus === undefined || commandStatus === receiptStatus);
}
...
function validProvenance(value) {
  const row = objectValue(value);
  return Object.keys(row).length === 2 && row.provider === 'static-site-importer/current-runtime' && ['completed', 'failed', 'pending_runtime'].includes(row.provider_status);
}

function validReceipt(row) {
  const allowed = ['schema', 'status', 'plan_hash', 'page_count', 'file_count', 'operation_count', 'loss_count', 'failure_code', 'reason_code', 'message', 'provider_totals', 'computed_layout_totals', 'operation_rows', 'loss_rows', 'truncated'];
  if (Object.keys(row).some((key) => !allowed.includes(key))) return false;
  if (row.schema !== 'static-site-importer/materialization-receipt/v1' || !['completed', 'failed', 'pending_runtime'].includes(row.status)) return false;
  if (row.status === 'pending_runtime') return Object.keys(row).every((key) => ['schema', 'status', 'reason_code', 'message'].includes(key)) && safeToken(row.reason_code, 80) && typeof row.message === 'string' && row.message.length <= 240;
...
function validCommandResult(value) {
  ...
  return Object.keys(row).every((key) => ['status', 'success', 'error_code', 'error_hash'].includes(key))
    && ['completed', 'failed', 'pending_runtime'].includes(row.status)
    && typeof row.success === 'boolean'
    && row.success === (row.status === 'completed')
    && typeof row.error_code === 'string'
    && (row.status === 'failed' ? safeToken(row.error_code, 80) : row.error_code === '')
...
function fixtureStatus(payloadCount, error, success, payload) {
