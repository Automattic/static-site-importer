...
function normalizeCollectedFixtureResult({ fixture, payloads, fixtureArtifactsDirectory, codeboxError, visualParity, liveWpParity, dependencyOverrides, sidecar }) {
  ...
  const diagnostics = attachDiagnosticLineage([...collectFixtureDiagnostics(merged, { visualParity: visualParityOptions }), ...runtimePresentationEvidence.diagnostics], payloads);
...
export function collectEditorPresentation(payload, matrixEvidence = null) {
  ...
  const report = objectValue(payload.import_report || payload.importReport || payload.report);
  const plan = objectValue(objectValue(report.blocks_engine || report.blocksEngine).wordpress_site_plan || objectValue(report.blocks_engine || report.blocksEngine).wordpressSitePlan);
  const evidencePlan = objectValue(objectValue(matrixEvidence).wordpress_site_plan || objectValue(matrixEvidence).wordpressSitePlan);
  const sourcePlanAssets = normalizeArray(plan.assets);
  const assets = sourcePlanAssets.length > 0 ? sourcePlanAssets : normalizeArray(evidencePlan.assets);
  const expectedIdentitiesComplete = sourcePlanAssets.length > 0
    ? sitePlanAssetsComplete(plan)
    : sitePlanAssetsComplete(evidencePlan);
  const expectedIdentities = [...new Set(assets.filter((asset) => {
  ...
  }).map((asset) => String(asset.content_hash || asset.contentHash || asset.payload_sha256 || '').toLowerCase()).filter((value) => /^[a-f0-9]{64}$/.test(value)))].sort();
...
function readFixturePayloadFiles(directory) {
  return ['validation-result.json', 'result.json', 'import-report.json', 'quality.json', 'blocks-engine-diagnostics.json', 'provider-submission-evidence.json', 'editor-validation.json', 'editor-validate-blocks.json', 'editor-open.json', 'editor-summary.json', 'editor-state.json', 'editor-validity.json', 'editor-canvas-summary.json', 'visual-compare.json', 'visual-diff.json', 'visual-parity.json', 'visual-explanation.json']
