...
function recipeStepFailureDiagnostic(failure, context = {}) {
  ...
  return compactObject({
    ...
    reason: diagnosticMessage(failure) || failure.error || failure.status || 'WP Codebox recipe step failed.',
    message: diagnosticMessage(failure) || 'WP Codebox recipe step failed.',
  });
}
...
function isTimeoutFailure(failure, timeoutClass) {
  const haystack = [timeoutClass, failure.kind, failure.code, failure.type, failure.message, failure.reason, failure.error, failure.status]
    .filter(Boolean)
    .join(' ');
