...
import {
  ...
  EDITOR_VISIBLE_PLACEHOLDER_KIND,
...
export function collectEditorValidationDiagnostics(payload) {
  ...
  for (const group of collectEditorCanvasPlaceholderGroups(payload)) {
    const count = numberValue(group.visible_count ?? group.visibleCount ?? group.count);
    if (count <= 0) {
      continue;
    }
    diagnostics.push({
      kind: EDITOR_VISIBLE_PLACEHOLDER_KIND,
      loss_class: EDITOR_VISIBLE_PLACEHOLDER_KIND,
      selector: group.selector || EDITOR_VISIBLE_PLACEHOLDER_SELECTORS[0],
      source_path: group.source_path || group.sourcePath || '',
      observed_output: firstString([group.first_match?.text, group.firstMatch?.text]),
      message: `Editor rendered ${count} visible imported group placeholder${count === 1 ? '' : 's'}.`,
    });
  }
...
export function collectEditorValidation(payload) {
  ...
  return compactObject({
    schema: EDITOR_VALIDATE_BLOCKS_SCHEMA,
    ...
    content_source: firstString([source.content_source, source.contentSource]),
    block_types_registered: pickCount(source.block_types_registered ?? source.blockTypesRegistered, 0),
    result_count: totalFromResults,
    results_complete: totalFromResults > 0 && results.every((block) => (
      typeof block.name === 'string'
      && typeof (block.isValid ?? block.is_valid ?? block.valid) === 'boolean'
...
function collectEditorCanvasPlaceholderGroups(payload) {
  const groups = [];
  for (const summary of collectEditorCanvasSummaries(payload)) {
    for (const group of normalizeArray(summary.selectorSummary?.groups || summary.selector_summary?.groups || summary.groups)) {
      if (String(group?.name || '').toLowerCase() === EDITOR_VISIBLE_PLACEHOLDER_SELECTOR_GROUP) {
        for (const selector of normalizeArray(group.selectors)) {
          groups.push({ ...selector, name: group.name });
        }
      }
    }
  }
