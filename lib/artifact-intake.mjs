...
import { GENERATED_ARTIFACT_METADATA_FILENAME } from './fixture-matrix/shared/constants.mjs';
...
function readWebsiteArtifact(directory) {
  for (const name of ['artifact.json', 'site-artifact.json', 'website-artifact.json', 'static-site-candidate.json']) {
...
function materializeWebsiteArtifact(artifact, fixtureDirectory) {
  ...
  if (artifact.compiler_limits && typeof artifact.compiler_limits === 'object' && !Array.isArray(artifact.compiler_limits)) {
    fs.writeFileSync(path.join(fixtureDirectory, GENERATED_ARTIFACT_METADATA_FILENAME), JSON.stringify({
      schema: 'static-site-importer/generated-artifact-metadata/v1',
      compiler_limits: artifact.compiler_limits,
    }));
  }
}
