 ...
 *
 * `viewport` is `exact` for a resting canvas. A selected canvas uses `width`,
 * because selecting a block can surrender canvas height to editor chrome such
 * as the contextual block toolbar. Width determines layout and still has to
 * match; height is recorded on the check as accounted-for chrome.
 */
export function evaluateEditorPresentation(map, measurements, { viewport = 'exact' } = {}) {
  if (!['exact', 'width'].includes(viewport)) throw new Error('Presentation viewport policy must be exact or width.');
  ...
  for (const surface of SURFACES) {
    ...
    for (const dimension of viewport === 'width' ? ['width'] : ['width', 'height']) {
...
export function presentationEvidencePassed(result) {
  try {
    ...
    for (const target of selectedTargets) {
      ...
      if (checks.length !== 1 || checks[0].status !== 'passed' || !Array.isArray(checks[0].affordances)) return false;
      if (evaluateEditorPresentation(result.map, { ...result.measurements, editor: checks[0].measurements }, { viewport: 'width' }).status !== 'passed') return false;
    }
...
/**
 * Record the resting canvas so a later selection can be told apart from it.
 * Gutenberg mutates existing nodes in place, so an unmarked element after
 * selection is an editor affordance the selection introduced.
 */
export async function markRestingCanvas(frame) {
  return frame.evaluate(() => {
    let marked = 0;
    for (const node of document.querySelectorAll('body, body *')) { node.setAttribute('data-ssi-resting', ''); marked += 1; }
    return marked;
  });
}

async function clearRestingCanvas(frame) {
  await frame.evaluate(() => { for (const node of document.querySelectorAll('[data-ssi-resting]')) node.removeAttribute('data-ssi-resting'); });
}

/**
 * Measure a selected canvas with the affordances selection introduced taken
 * out of layout, and restore them afterwards. Neutralization only ever touches
 * nodes that did not exist at rest, so a restyled or resized piece of real
 * content still reaches the comparison and still fails.
 */
export async function measureSelectedPresentation(frame, map) {
  const affordances = await frame.evaluate(() => {
    const introduced = [...document.querySelectorAll('body *:not([data-ssi-resting])')];
    const inventory = introduced.map(node => {
      const rect = node.getBoundingClientRect();
      const style = getComputedStyle(node);
      return { tag: node.tagName.toLowerCase(), class_name: (node.className?.toString?.() ?? '').slice(0, 120), width: rect.width, height: rect.height, in_flow: style.position === 'static' };
    });
    for (const node of introduced) { node.setAttribute('data-ssi-affordance', node.style.display); node.style.display = 'none'; }
    return inventory;
  });
  try {
    return { measurements: await measurePresentation(frame, map, 'editor'), affordances };
  } finally {
    await frame.evaluate(() => {
      for (const node of document.querySelectorAll('[data-ssi-affordance]')) { node.style.display = node.getAttribute('data-ssi-affordance'); node.removeAttribute('data-ssi-affordance'); }
    });
  }
}
...
export async function captureEditorPresentation(browser, editorPage, options, map, viewport) {
  ...
  try {
    ...
    result.resting_canvas_nodes = await markRestingCanvas(frame);
    for (const target of map.targets.filter(target => ['heading', 'label', 'field', 'submit'].includes(target.role))) {
      ...
      await frame.evaluate(() => new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve))));
      const { measurements: selected, affordances } = await measureSelectedPresentation(frame, map);
      const check = evaluateEditorPresentation(map, { ...result.measurements, editor: selected }, { viewport: 'width' });
      const chrome = { canvas_height: selected.viewport.height, resting_canvas_height: result.measurements.editor.viewport.height };
      result.selection_checks.push({ target: target.id, ...check, affordances, chrome, measurements: selected });
      if (check.status !== 'passed') {
        result.status = 'failed';
        for (const finding of check.findings) result.findings.push({ ...finding, state: 'selected', selected_target: target.id });
      }
    ...
    await editorPage.evaluate(() => window.wp.data.dispatch('core/block-editor').clearSelectedBlock());
    await clearRestingCanvas(frame);
