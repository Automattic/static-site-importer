...
async function measureMarkedElements( page ) {
	const measurement = await page.evaluate( attribute => {
		...
		// Hosts are measured by their content box: placement tracks span the
		// area inside the host's padding and border, not its outer edge.
		const hostSet = new Set( [ ...itemsByHost.keys() ] );
		const rows = marked.map( element => {
			const rect = element.getBoundingClientRect();
			const path = element.getAttribute( attribute );
			if ( ! hostSet.has( path ) ) {
				return [ path, { x: rect.x, y: rect.y + window.scrollY, width: rect.width, height: rect.height } ];
			}
